...
<div data-link-id="<?= $data->link->link_id ?>" class="col-12 my-2">
    ...
    <a href="<?= $data->link->location_url . $data->link->utm_query ?>" data-link-url="<?= $data->link->url ?>" target="_blank">
        <img src="<?= substr($data->link->settings->image, 0, 4) === "http" ? $data->link->settings->image : SITE_URL . UPLOADS_URL_PATH . 'block_images/' . $data->link->settings->image ?>" class="img-fluid rounded" />
    </a>
    <?php else: ?>
    <img src="<?= substr($data->link->settings->image, 0, 4) === "http" ? $data->link->settings->image : SITE_URL . UPLOADS_URL_PATH . 'block_images/' . $data->link->settings->image ?>" class="img-fluid rounded" />
    <?php endif ?>
