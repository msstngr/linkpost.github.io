...
class Link extends Controller {
    ...
    public function index() {
        ...
        $this->is_qr = isset($this->params[1]) && $this->params[1] == 'qr' ? $this->params[1] : null;
        ...
        $this->link->full_url = $domain_id && !isset($_GET['link_id']) ? Router::$data['domain']->scheme . Router::$data['domain']->host . '/' . $this->link->url : url($this->link->url);
    ...
    private function create_statistics() {
        ...
        database()->query("UPDATE `links` SET `clicks` = `clicks` + 1 WHERE `link_id` = {$this->link->link_id}");
    ...
    public function process_biolink() {
        ...
        if(is_null($cache_instance->get())) {
            ...
            $result = database()->query("SELECT * FROM `links` WHERE `biolink_id` = {$this->link->link_id} AND `type` = 'biolink' AND `subtype` <> 'base' AND `is_enabled` = 1 ORDER BY `order` ASC");
            $links = [];
        ...
        if($this->user->plan_settings->seo) {
            ...
            Meta::set_social_image(!empty($this->link->settings->seo->image) ? SITE_URL . UPLOADS_URL_PATH . 'block_images/' . $this->link->settings->seo->image : null);
        }
