...
class ApiProjects extends Controller {
    ...
    private function post() {
        ...
        $total_rows = db()->where('user_id', $this->api_user->user_id)->getValue('projects', 'count(`website_id`)');

        if($this->api_user->plan_settings->projects_limit != -1 && $total_rows >= $this->api_user->plan_settings->projects_limit) {
            $this->response_error(language()->project_create_modal->error_message->projects_limit, 401);
        }
